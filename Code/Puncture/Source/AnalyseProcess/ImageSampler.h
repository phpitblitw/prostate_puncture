...
//********输入采样坐标均为WLD坐标系下，考虑了分辨率，单位为mm，内部转换为IJK坐标系***
	...
	public:
		...
		void SetProbeOffset(float rightOffset, float upOffset, float moveOffset);  //设置B超探头，矢状面中心，相对于横断面中心，在rightDir、upDir、moveDir三个方向上的偏移量(mm)
	...
	private:
		...
		float m_fSagittalOffset[3];		//B超探头，矢状面中心，相对于横断面中心，在rightDir、upDir、moveDir三个方向上的偏移量(mm)
