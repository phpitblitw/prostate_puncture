...
namespace ANALYSEPROCESS
{
	...
	public:
		...
		BOOL SetBaseUSPosition(NDIOPERATOR::Attitude aBaseUS);	//设置base处，超声位置参数
		BOOL SetBaseMRIPosition(AnalyseConfig *config);	//设置base处，MRI模拟采样位置参数
		BOOL CalculateTransformMatrix();	//根据base处超声和MRI模拟采样的未知参数，计算转换矩阵
		BOOL SetCurUSPosition(NDIOPERATOR::Attitude aCurUS);	//设置当前超声位置
	...
	private:
		Matrix ConstructAttitude2Matrix(NDIOPERATOR::Attitude attitude);	//根据attitude计算4*4变换矩阵
	};
}